...
2. `if (isset($_POST['ctype']) && isset($_POST['is_num'])) { ... }`
   ...
   php中，`ctype_alpha`检查字符串是否只包含字母，而`is_numeric`检查字符串是否为数字。处理 hash 字符串时，PHP 会将每一个以 0E 开头的哈希值解释为 0（0的xxx次方），那么只要传入的不同字符串经过哈希以后是以 0E 开头的，那么 PHP 会认为它们相同。因此我们可以利用这一点来绕过这个检查点。
    ...
    <p align="center">
    <img src='image/[NSSRound_16 Basic]了解过PHP特性吗-md5_collision_bypass.png'>
    </p>
...
3. `if (isset($_114) && intval($_114) > 114514 && strlen($_114) <= 3) { ... }`
   ...
   <p align="center">
   <img src="image/[NSSRound_16 Basic]了解过PHP特性吗-scientific_notation_bypass.png">
   </p>
...
4. `if (is_array($arr4y)) { ... }`
   ...
   <p align="center">
   <img src="image/[NSSRound_16 Basic]了解过PHP特性吗-array_search_bypass.png">
   </p>

传参获得如下提示
<p align="center">
<img src="image/[NSSRound_16 Basic]了解过PHP特性吗-rce_code_reveal.png">
</p>
...
执行后如下
<p align="center">
<img src="image/[NSSRound_16 Basic]了解过PHP特性吗-rce_ls_output.png">
</p>
可以看到成功执行了代码
接下来尝试读取flag
```
...
```
<p align="center">
<img src="image/[NSSRound_16 Basic]了解过PHP特性吗-flag_cat_output.png">
</p>
